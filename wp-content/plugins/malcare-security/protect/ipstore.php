...
class MCProtectIpstore_V577 {
	...
	function __construct($storage_type = MCProtectIpstore_V577::STORAGE_TYPE_DB) {
		$this->storage_type = $storage_type;
		if ($this->storage_type == MCProtectIpstore_V577::STORAGE_TYPE_FS) {
			$this->storage = new MCProtectIpstoreFS_V577();
		} else {
			$this->storage = new MCProtectIpstoreDB_V577();
		}
	}

	public static function uninstall() {
		MCProtectIpstoreDB_V577::uninstall();
	}

	public function isLPIPBlacklisted($ip) {
		if ($this->storage_type == MCProtectIpstore_V577::STORAGE_TYPE_DB) {
