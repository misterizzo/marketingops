...
class MCProtectIpstore_V573 {
	...
	function __construct($storage_type = MCProtectIpstore_V573::STORAGE_TYPE_DB) {
		$this->storage_type = $storage_type;
		if ($this->storage_type == MCProtectIpstore_V573::STORAGE_TYPE_FS) {
			$this->storage = new MCProtectIpstoreFS_V573();
		} else {
			$this->storage = new MCProtectIpstoreDB_V573();
		}
	}

	public static function uninstall() {
		MCProtectIpstoreDB_V573::uninstall();
	}

	public function isLPIPBlacklisted($ip) {
		if ($this->storage_type == MCProtectIpstore_V573::STORAGE_TYPE_DB) {
