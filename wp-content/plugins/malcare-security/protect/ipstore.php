...
class MCProtectIpstore_V593 {
	...
	function __construct($storage_type = MCProtectIpstore_V593::STORAGE_TYPE_DB) {
		$this->storage_type = $storage_type;
		if ($this->storage_type == MCProtectIpstore_V593::STORAGE_TYPE_FS) {
			$this->storage = new MCProtectIpstoreFS_V593();
		} else {
			$this->storage = new MCProtectIpstoreDB_V593();
		}
	}

	public static function uninstall() {
		MCProtectIpstoreDB_V593::uninstall();
	}

	public function isLPIPBlacklisted($ip) {
		if ($this->storage_type == MCProtectIpstore_V593::STORAGE_TYPE_DB) {
