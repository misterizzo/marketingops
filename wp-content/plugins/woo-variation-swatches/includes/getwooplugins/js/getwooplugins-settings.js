...
const GWPDirtySettingSaveWarningHandler = (event) => {
	// Recommended
	event.preventDefault()

	event.returnValue = getwooplugins_settings_params.i18n_nav_warning
	return getwooplugins_settings_params.i18n_nav_warning
}
		...
		$(function () {
			...
			$('.getwooplugins-settings-form :input:not(.no-track)').on('change', function () {
				if (!changed) {
					...
					window.addEventListener('beforeunload', GWPDirtySettingSaveWarningHandler)
					changed = true
				}
			})

			$('.submit :input').on('click', function () {
				window.removeEventListener('beforeunload', GWPDirtySettingSaveWarningHandler)
			})
		})
