<?php
/**
 * Handles all server side logic for the ld-quiz-list Gutenberg Block. This block is functionally the same
 * as the ld_quiz_list shortcode used within LearnDash.
 *
 * @package LearnDash
 * @since 2.5.9
 */
...
if ( ( class_exists( 'LearnDash_Gutenberg_Block' ) ) && ( ! class_exists( 'LearnDash_Gutenberg_Block_Quiz_List' ) ) ) {
	/**
	 * Class for handling LearnDash Quiz List Block
	 */
	class LearnDash_Gutenberg_Block_Quiz_List extends LearnDash_Gutenberg_Block {
		...
		public function __construct() {
			...
			$this->block_attributes = array(
				...
				'quiz_category_name'    => array(
					'type' => 'string',
				),
				'quiz_cat'              => array(
					'type' => 'string',
				),
				'quiz_categoryselector' => array(
					'type' => 'boolean',
				),
				'quiz_tag'              => array(
					'type' => 'string',
		...
		public function learndash_block_markers_shortcode_atts_filter( $block_attributes = array(), $shortcode_slug = '', $block_slug = '', $content = '' ) {
			if ( $shortcode_slug === $this->shortcode_slug ) {
				...
				if ( ( isset( $block_attributes['quiz_categoryselector'] ) ) && ( true === $block_attributes['quiz_categoryselector'] ) ) {
					$block_attributes['quiz_categoryselector'] = 'true';
				}
...
new LearnDash_Gutenberg_Block_Quiz_List();
