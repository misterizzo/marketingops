				...
				if ( get_option( 'job_manager_enable_types' ) ) {
					...
					if ( $job_types ) {
						foreach ( $job_types as $job_type ) {
							$meta[] = '<span class="embeddable-job-widget-listing-job-type">' . esc_html( $job_type->name ) . '</span>';
						}
					}
				}

				if ( $data = get_the_job_location() ) {
					$meta[] = '<span class="embeddable-job-widget-listing-job-location">' . esc_html( $data ) . '</span>';
				}
				if ( $data = get_the_company_name() ) {
					$meta[] = '<span class="embeddable-job-widget-listing-job-company">' . esc_html( $data ) . '</span>';
				}
