 ...
 * Tested up to: 6.4
 * Requires PHP: 7.4
 * Text Domain: wp-job-manager-embeddable-job-widget
...
class WP_Job_Manager_Embeddable_Job_Widget {
	...
	public function __construct() {
		// Define constants
		define( 'JOB_MANAGER_EMBEDDABLE_JOB_WIDGET_VERSION', '1.1.6' );
		define( 'JOB_MANAGER_EMBEDDABLE_JOB_WIDGET_PLUGIN_DIR', untrailingslashit( plugin_dir_path( __FILE__ ) ) );
		define( 'JOB_MANAGER_EMBEDDABLE_JOB_WIDGET_PLUGIN_URL', untrailingslashit( plugins_url( '', ( __FILE__ ) ) ) );
	...
	public function version_check() {
		if ( ! class_exists( 'WP_Job_Manager' ) || ! defined( 'JOB_MANAGER_VERSION' ) ) {
			...
			$this->display_error( sprintf( __( '<em>WP Job Manager - Embeddable Job Widget</em> requires WP Job Manager %1$s (you are using %2$s).', 'wp-job-manager-embeddable-job-widget' ), self::JOB_MANAGER_CORE_MIN_VERSION, JOB_MANAGER_VERSION ) );
		}
	}
	...
	public function frontend_scripts() {
		wp_register_script( 'embeddable-job-widget', JOB_MANAGER_EMBEDDABLE_JOB_WIDGET_PLUGIN_URL . '/assets/dist/js/form.js', array( 'jquery', 'select2' ), JOB_MANAGER_EMBEDDABLE_JOB_WIDGET_VERSION, true );
		wp_enqueue_style( 'embeddable-job-widget-frontend', JOB_MANAGER_EMBEDDABLE_JOB_WIDGET_PLUGIN_URL . '/assets/dist/css/frontend.css' );
		...
		wp_localize_script(
			...
			'embeddable_job_widget_form_args',
			array(
				'code'       => $code,
				'css'        => $css,
				'theme_dark' => '',
	...
	public function job_widget_js() {
		if ( ! empty( $_GET['embed'] ) && 'wp_job_manager_widget' === $_GET['embed'] ) {
			...
			$per_page   = absint( $_GET['per_page'] ?? 5 );
			$jobs       = get_job_listings(
				apply_filters(
					'job_manager_embeddable_job_widget_query_args',
					array(
	...
	private function is_shortcode_page() {
		...
		return null !== $content && has_shortcode( $content, 'embeddable_job_widget_generator' );
	}
}
