	...
	public function close(&$ahtml, &$ihtml)
	{
		...
		if (isset($this->mpdf->table[1][1]['overflow']) && $this->mpdf->table[1][1]['overflow'] === 'visible') {
			if ($this->mpdf->kwt || $this->mpdf->table_rotate || $this->mpdf->table_keep_together || $this->mpdf->ColActive) {
				...
				$this->mpdf->table_rotate = 0;
				$this->mpdf->table_keep_together = false;
				//throw new \LearnDash\Certificate_Builder\Mpdf\MpdfException("mPDF Warning: You cannot use CSS overflow:visible together with any of these functions:
		...
		if ($this->mpdf->table[1][1]['overflow'] === 'visible') {
			if ($maxrowheight > $fullpage) {
				throw new \LearnDash\Certificate_Builder\Mpdf\MpdfException('mPDF Warning: A Table row is greater than available height. You cannot use CSS overflow:visible');
			}
