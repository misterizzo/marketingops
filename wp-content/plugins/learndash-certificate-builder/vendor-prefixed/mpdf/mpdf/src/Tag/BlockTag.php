...
use LearnDash\Certificate_Builder\Mpdf\Conversion\DecToAlpha;
use LearnDash\Certificate_Builder\Mpdf\Conversion\DecToRoman;
	...
	public function open($attr, &$ahtml, &$ihtml)
	{
		...
		if ((isset($p['POSITION'])
			...
			if ($this->mpdf->inFixedPosBlock) {
				throw new \LearnDash\Certificate_Builder\Mpdf\MpdfException('Cannot nest block with position:fixed or position:absolute');
			}
		...
		if ($currblk['inner_width'] < $mw) {
			...
			// if ($currblk['inner_width'] < $mw) { throw new \LearnDash\Certificate_Builder\Mpdf\MpdfException("DIV is too narrow for text to fit!"); }
		}
