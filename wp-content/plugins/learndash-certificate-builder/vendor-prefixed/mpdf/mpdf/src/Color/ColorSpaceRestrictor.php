...
class ColorSpaceRestrictor
{
	 ...
	 * @param \LearnDash\Certificate_Builder\Mpdf\Color\ColorModeConverter $colorModeConverter
	 * @param int $mode
	 */
	...
	private function restrictSpotColorSpace($c, &$PDFAXwarnings = [])
	{
		if (!isset($this->mpdf->spotColorIDs[$c[1]])) {
				throw new \LearnDash\Certificate_Builder\Mpdf\MpdfException('Error: Spot colour has not been defined - ' . $this->mpdf->spotColorIDs[$c[1]]);
		}
