...
namespace LearnDash\Certificate_Builder\Mpdf\Image;

use LearnDash\Certificate_Builder\Mpdf\AssetFetcher;
...
use LearnDash\Certificate_Builder\Mpdf\Gif\Gif;
use LearnDash\Certificate_Builder\Mpdf\Language\LanguageToFontInterface;
use LearnDash\Certificate_Builder\Mpdf\Language\ScriptToLanguageInterface;
...
class ImageProcessor implements \LearnDash\Certificate_Builder\Psr\Log\LoggerAwareInterface
{
	...
	/**
	 * @var \LearnDash\Certificate_Builder\Mpdf\Image\ImageTypeGuesser
	 */
	private $guesser;
	...
	public function getImage(&$file, $firstTime = true, $allowvector = true, $orig_srcpath = false, $interpolation = false)
	{
		...
		if (!$data) {
			...
			} catch (\LearnDash\Certificate_Builder\Mpdf\Exception\AssetFetchingException $e) {
	...
	private function gzCompress($data)
	{
		if (!function_exists('gzcompress')) {
			throw new \LearnDash\Certificate_Builder\Mpdf\MpdfException('gzcompress is not available. install ext-zlib extension.');
		}
	...
	private function imageError($file, $firstTime, $msg)
	{
		...
		if ($firstTime && ($this->mpdf->showImageErrors || $this->mpdf->debug)) {
			throw new \LearnDash\Certificate_Builder\Mpdf\MpdfImageException(sprintf('%s (%s)', $msg, substr($file, 0, 256)));
		}
	...
	public function processJpg($data, $file, $firstTime, $interpolation)
	{
		...
		if ($a[2] === 'DeviceCMYK' && ($this->mpdf->restrictColorSpace === 2 || ($this->mpdf->PDFA && $this->mpdf->restrictColorSpace !== 3))) {
			...
			if (!function_exists('gd_info')) {
				throw new \LearnDash\Certificate_Builder\Mpdf\MpdfException(sprintf('JPG image may not use CMYK color space (%s).', $file));
			}
	...
	public function processPng($data, $file, $firstTime, $interpolation)
	{
		...
		if ($firstTime && ($colspace === 'DeviceRGB' || $colspace === 'Indexed') && ($this->mpdf->PDFX || $this->mpdf->restrictColorSpace === 3)) {
			...
			if ($pngalpha) {
				if ($this->mpdf->PDFA) {
					throw new \LearnDash\Certificate_Builder\Mpdf\MpdfException(sprintf('PDFA1-b does not permit images with alpha channel transparency (%s).', $file));
				}
