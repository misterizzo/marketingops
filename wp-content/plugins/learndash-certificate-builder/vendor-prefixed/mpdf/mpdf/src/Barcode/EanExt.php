 ...
 */
class EanExt extends \LearnDash\Certificate_Builder\Mpdf\Barcode\AbstractBarcode implements \LearnDash\Certificate_Builder\Mpdf\Barcode\BarcodeInterface
{
	...
	private function init($code, $length = 5)
	{
		...
		if ($length == 2) {
			...
			throw new \LearnDash\Certificate_Builder\Mpdf\Barcode\BarcodeException(sprintf('Invalid EAN barcode value "%s"', $code));
		}
