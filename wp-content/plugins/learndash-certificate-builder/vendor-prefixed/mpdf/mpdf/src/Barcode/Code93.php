 ...
 */
class Code93 extends \LearnDash\Certificate_Builder\Mpdf\Barcode\AbstractBarcode implements \LearnDash\Certificate_Builder\Mpdf\Barcode\BarcodeInterface
{
	...
	private function init($code)
	{
		...
		for ($i = 0; $i < $clen; ++$i) {
			if (ord($code[$i]) > 127) {
				throw new \LearnDash\Certificate_Builder\Mpdf\Barcode\BarcodeException(sprintf('Invalid character "%s" in CODE93 barcode value "%s"', $code[$i], $code));
			}
		...
		for ($i = 0; $i < $clen; ++$i) {
			...
			if (!isset($chr[$char])) {
				// invalid character
				throw new \LearnDash\Certificate_Builder\Mpdf\Barcode\BarcodeException(sprintf('Invalid CODE93 barcode value "%s"', $code));
			}
			for ($j = 0; $j < 6; ++$j) {
				if (($j % 2) == 0) {
					$t = true; // bar
