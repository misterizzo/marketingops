 ...
 */
class I25 extends \LearnDash\Certificate_Builder\Mpdf\Barcode\AbstractBarcode implements \LearnDash\Certificate_Builder\Mpdf\Barcode\BarcodeInterface
{
	...
	private function init($code, $printRatio, $checksum)
	{
		...
		for ($i = 0; $i < $clen; $i = ($i + 2)) {
			...
			if ((!isset($chr[$charBar])) or (!isset($chr[$charSpace]))) {
				// invalid character
				throw new \LearnDash\Certificate_Builder\Mpdf\Barcode\BarcodeException(sprintf('Invalid I25 barcode value "%s"', $code));
			}
