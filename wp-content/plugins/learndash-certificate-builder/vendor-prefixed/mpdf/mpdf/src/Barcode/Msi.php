 ...
 */
class Msi extends \LearnDash\Certificate_Builder\Mpdf\Barcode\AbstractBarcode implements \LearnDash\Certificate_Builder\Mpdf\Barcode\BarcodeInterface
{
	...
	private function init($code, $checksum)
	{
		...
		for ($i = 0; $i < $clen; ++$i) {
			...
			if (!isset($chr[$digit])) {
				// invalid character
				throw new \LearnDash\Certificate_Builder\Mpdf\Barcode\BarcodeException(sprintf('Invalid character "%s" in MSI barcode value "%s"', $digit, $code));
			}
			$seq .= $chr[$digit];
		}
