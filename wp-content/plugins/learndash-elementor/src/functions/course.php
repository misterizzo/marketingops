 ...
 * @return void
 */
function learndash_elementor_show_course_content_listing( $atts = array() ): void {
	...
	?><div class="<?php echo esc_attr( learndash_get_wrapper_class( absint( $course_id ) ) ); ?>">
	<?php
				...
				if ( $has_topics || $has_lesson_quizzes ) :
						...
						array_filter(
							array_map(
								function ( $lesson_id ) use ( $user_id, $course_id ) {
									$topics = learndash_get_topic_list(
										Cast::to_int( $lesson_id ),
										Cast::to_int( $course_id )
									);

									$quizzes = learndash_get_lesson_quiz_list(
										Cast::to_int( $lesson_id ),
										Cast::to_int( $user_id ),
										Cast::to_int( $course_id )
									);
					...
					?>
						<button
							...
							data-ld-expand-text="<?php echo esc_attr_e( 'Expand All', 'learndash-elementor' ); ?>"
							data-ld-collapse-text="<?php echo esc_attr_e( 'Collapse All', 'learndash-elementor' ); ?>"
						>
							<span class="ld-icon-arrow-down ld-icon"></span>
							<span class="ld-text"><?php echo esc_html_e( 'Expand All', 'learndash-elementor' ); ?></span>
 ...
 * @return void
 */
function learndash_elementor_activity_start_step( $user_id = 0, $step_id = 0, $course_id = 0 ): void {
	if ( ( ! empty( $user_id ) ) && ( ! empty( $step_id ) ) && ( ! empty( $course_id ) ) ) {
		$user_id   = absint( $user_id );
		$step_id   = absint( $step_id );
		$course_id = absint( $course_id );
...
function learndash_elementor_user_step_access_state( $state = '', $user_id = 0, $step_id = 0, $course_id = 0 ) {
	...
	if ( ( ! empty( $state ) ) /* && ( ! empty( $user_id ) ) */ && ( ! empty( $step_id ) ) && ( ! empty( $course_id ) ) ) {
				...
				if ( $lesson_progression_enabled ) {
					switch ( get_post_type( $step_id ) ) {
						...
						case learndash_get_post_type_slug( 'quiz' ):
							...
							// We check if the content should be inaccessible.
		...
		if (
			get_post_type( $step_id ) === learndash_get_post_type_slug( LDLMS_Post_Types::QUIZ )
			&& ! learndash_is_quiz_accessable( null, $step_post, true, $course_id )
		) {
			$step_state['show_content'] = false;
		}
