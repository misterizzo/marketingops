<?php
/**
 * Class for DOMDocument drop-in replacement.
 *
...
/**
 * Drop in replacement for DOMDocument that is secure against XML eXternal Entity (XXE)
 * Injection.
 *
...
class WC_Safe_DOMDocument extends DOMDocument {
	/**
	 * When called non-statically (as an object method) with malicious data,
	 * no Exception is thrown, but the object is emptied of all DOM nodes.
	 *
	 * @param string $filename The path to the XML document.
	 * @param int    $options  Bitwise OR of the libxml option constants.
	 *
	 * @see http://us3.php.net/manual/en/libxml.constants.php
	 *
	 * @return bool|DOMDocument true on success, false on failure. If called statically
	 *                          (E_STRICT error), returns DOMDocument on success.
	 */
	public function load( $filename, $options = 0 ) {
		if ( '' === $filename ) {
			// If an empty string is passed as the filename or an empty file is
			// named, a warning will be generated. This warning is not generated
			// by libxml and cannot be handled using libxml's error handling functions.
			trigger_error( 'WC_Safe_DOMDocument::load(): Empty string supplied as input', E_USER_WARNING );
			return false;
		}

		if ( ! is_file( $filename ) || ! is_readable( $filename ) ) {
			// This warning probably would have been generated by libxml and could
			// have been handled using libxml's error handling functions.
			// In WC_Safe_DOMDocument, however, we catch it before libxml, so it can't.
			// The alternative is to let file_get_contents() handle the error, but
			// that's annoying.
			trigger_error( 'WC_Safe_DOMDocument::load(): I/O warning : failed to load external entity "' . esc_html( $filename ) . '"', E_USER_WARNING );
	 ...
	 * @see http://us3.php.net/manual/en/libxml.constants.php.
	 *
	 * @return bool|DOMDocument true on success, false on failure.  If called
	 *                          statically (E_STRICT error), returns DOMDocument on success.
	 */
	public function loadXML( $source, $options = 0 ) {
		if ( '' === $source ) {
			// If an empty string is passed as the source, a warning will be generated.
			// This warning is not generated by libxml and cannot be handled using
