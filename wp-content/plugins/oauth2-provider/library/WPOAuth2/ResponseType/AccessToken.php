...
class AccessToken implements AccessTokenInterface {
	 ...
	 *                                                             <code> $config = array( 'token_type' =>
	 *                                                             'bearer',              // token type
	 *                                                             identifier 'access_lifetime' => 3600,
	...
	public function createAccessToken( $client_id, $user_id, $scope = null, $includeRefreshToken = true ) {
		...
		do_action( 'wo_before_set_access_token', array( $client_id, $user_id ) );
...
*/
		if ( ! method_exists( $this->tokenStorage, 'unsetAccessToken' ) ) {
			throw new \RuntimeException(
				sprintf(
					'Token storage %s must implement unsetAccessToken method',
					get_class( $this->tokenStorage )
				)
			);
		}
