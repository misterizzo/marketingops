...
class AuthorizeController implements AuthorizeControllerInterface {
	 ...
	 *                                                      <code> $config = array( 'allow_implicit' =>
	 *                                                      false,            // if the controller should
	 *                                                      allow the "implicit" grant type
	 *                                                      'enforce_state'  => true              // if
	 *                                                      the controller should require the "state"
	 *                                                      parameter 'require_exact_redirect_uri' =>
	 *                                                      true, // if the controller should require an
	 *                                                      exact match on the "redirect_uri" parameter
	 *                                                      'redirect_status_code' => 302,        // HTTP
	 *                                                      status code to use for redirect responses );
	 *                                                      </code>
	 *
	...
	public function handleAuthorizeRequest( RequestInterface $request, ResponseInterface $response, $is_authorized, $user_id = null ) {
		...
		/*
		* Handle the PCKE request if present
		* Proof Key for Code Exchange
		*
		* @todo look into storing the challenge and method in the code itself. It may be better to store this on the server instead. To many eyes on the code base so obfuscation is not an option making logic hard to secure.
		* @todo Look into moving this to $this->buildAuthorizeParameters??
		* @todo make code_challenge_method default follow the plugin default settings
		*
		* @since 4.0.0
		* @link https://tools.ietf.org/html/rfc7636
		*/
		if ( ! empty( $request->query( 'code_challenge' ) ) ) {
			$params['code_challenge']        = $request->query( 'code_challenge' );
			$params['code_challenge_method'] = empty( $request->query( 'code_challenge_method' ) ) ? 'plain' : $request->query( 'code_challenge_method' );
		}
