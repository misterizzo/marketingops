...
class User_Memberships extends Controller {
	...
	public function create_item( $request ) {
		...
		try {
			...
			foreach ( [ 'customer_id', 'plan_id' ] as $required_param ) {
			...
			try {
				$user_membership = wc_memberships_create_user_membership( [ 'plan_id' => $membership_plan->get_id(), 'user_id' => $customer->ID ] );
			} catch (\Exception $e) {
	...
	private function set_user_membership_data( $user_membership, $request ) {
		...
		foreach ( $dates as $date_id ) {
			...
			if ( isset( $request[ $date_id ] ) ) {
				...
				if ( is_string( $date_value) && is_callable( [ $user_membership, $set_date ] ) ) {
	...
	public function get_collection_params() {
		...
		$params['status'] = [
			...
			'type'              => 'string',
			'enum'              => array_merge( ['any'], wc_memberships_get_user_membership_statuses( false, false ) ),
	...
	protected function prepare_items_query_args( $request ) {
		...
		if ( is_string( $request['status'] ) && 'any' !== $request['status'] ) {
		...
		// the following code is just to account a case introduced by the memberships code block which should be removed in future versions as it shouldn't interact with the API in this way
		if ( ! empty( $request['wc_memberships_block'] ) ) {

			// we want to make sure that "private" plans are not queried accidentally while filtering members
			if ( isset( $plan ) && $plan instanceof \WC_Memberships_Membership_Plan ) {

				if ( ! $plan->is_public() ) {
					$query_args['post_parent'] = 0;
				}

			} elseif ( isset( $plan_id ) && is_numeric( $plan_id ) ) {
				...
				foreach ( wc_memberships_get_membership_plans( ['post_status' => 'any'] ) as $plan ) {
					if ( ! $plan->is_public() ) {
						$private_plans[] = $plan->get_id();
					}
				}

				if ( ! empty( $private_plans ) ) {
					$query_args['post_parent__not_in'] = $private_plans;
				}
			}
		}
	...
	protected function get_allowed_query_vars() {
		...
		$allowed_query_vars = parent::get_allowed_query_vars();
		$allowed_query_vars[] = 'post_status';
		$allowed_query_vars[] = 'author__in';
	...
	/**
	 * Gets a list of user IDs by a search term to use in user meta.
	 *
	 * @internal
	 *
	 * @since 1.23.0
	 ...
	 * @return array user ids list
	 */
	public function get_users_for_search( string $search_string = '' ) : array {
		global $wpdb;

		$users_query = $wpdb->prepare( "SELECT DISTINCT * FROM {$wpdb->usermeta} WHERE meta_value LIKE %s", '%' . $wpdb->esc_like( $search_string ) . '%' );

		return array_unique( array_column( $wpdb->get_results( $users_query ), 'user_id' ) );
	}
	 ...
	 * @param null|\WP_REST_Request $request optional request object
	...
	public function get_formatted_item_data( $user_membership, $request = null ) {
		...
		if ( $user_membership instanceof \WC_Memberships_User_Membership ) {
			...
			$data            = [
				...
				'plan_name'          => $plan ? $plan->get_name() : null,
			...
			// append customer data if present in the request
			// @TODO this was added solely to support the WooCommerce Memberships block, but it should be removed in future versions and oved to an AJAX query instead {unfulvio 2023-01-23}
			if ( ! empty( $request['customer_data'] ) ) {
				...
				try {
					...
					$data['customer_data'] = [
						...
						'bio'        => $user ? $user->get( 'description' ) : '',
						'avatar'     => get_avatar_url( $data['customer_id'] ) ?: null,
					];
	...
	public function get_endpoint_args_for_item_schema( $method = \WP_REST_Server::CREATABLE ) {
		...
		if ( in_array( $method, [ \WP_REST_Server::CREATABLE, \WP_REST_Server::EDITABLE ], true ) ) {
			...
			foreach ( array_keys( $endpoint_args ) as $property ) {
				...
				if ( in_array( $property, ['customer_id', 'plan_id'], true ) ) {
