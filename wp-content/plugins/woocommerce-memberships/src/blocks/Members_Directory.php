 ...
 *
 * @TODO this component needs to be refactored to be more consistent with the shortcode version {unfulvio 2024-02-06}
 * Also: the correct wording should be Member Directory and not Members Directory
 *
 * @since 1.23.0
 */
class Members_Directory extends Block implements Dynamic_Content_Block {
	 ...
	 *
	 * Displays the Membership Directory.
	 *
	 * @internal
	 *
	 * @since 1.23.0
	...
	public function render( $attributes, $content ) {
		...
		// sanity check: if the directory is not enabled, return an empty string
		...
		$container_style = "--wcm-btn-color:{$attributes[ 'btnColor' ]};--wcm-btn-hover-color:{$attributes[ 'btnHoverColor' ]};";

		ob_start();

		?>
		<div
			class="wc-memberships-directory-container wcm-directory-front-end align<?php echo esc_attr( $attributes[ 'align' ] ); ?>"
			data-directory-id='<?php echo esc_attr( wp_unique_id( 'wcm-directory-' ) ); ?>'
			data-directory-data='<?php echo json_encode( $attributes ); ?>'
			style="<?php echo $container_style; ?>">

			<div class="wmc-loader">
				<div class="wcm-spinner">
			...
			<div class="wc-memberships-directory-filter-wrapper" >
				...
				<div class="search-wrapper">
					<input type="text"
					       placeholder="<?php esc_attr_e( 'Search...', 'woocommerce-memberships' ); ?>"
					       class="wcm-input wcm-search-input" />
					<input type="button" class="wcm-btn wcm-search-btn" />
				</div>

				<div class="filter-wrapper">

					<select class="wcm-select wcm-input wcm-plans" multiple data-placeholder="<?php esc_attr_e( 'All Plans', 'woocommerce-memberships' ); ?>">
						<option value=""><?php esc_html_e( 'All Plans', 'woocommerce-memberships' ); ?></option>
						<?php foreach ( wc_memberships_get_membership_plans( [ 'post__in' => $attributes[ 'membershipPlans' ] ] ) as $membership_plan ) : ?>
							<option value="<?php echo esc_attr( $membership_plan->get_id() ); ?>" ><?php echo esc_html( $membership_plan->get_name() ) ?></option>
						<?php endforeach; ?>
					</select>

					<select class="wcm-select wcm-input wcm-status" multiple data-placeholder="<?php esc_attr_e( 'All Statuses', 'woocommerce-memberships' ); ?>">
						...
						foreach ( wc_memberships_get_user_membership_statuses() as $status_key => $membership_status ) :
							...
							if ( ! empty( $attributes[ 'membershipStatus' ] )  ) :
								if ( in_array( $status_key, $attributes[ 'membershipStatus' ] ) ) :
									?><option value="<?php echo esc_attr( $status_key ); ?>" ><?php echo esc_html( $membership_status['label'] ); ?></option><?php
								endif;
							else :
								?><option value="<?php echo esc_attr( $status_key ); ?>" ><?php echo esc_html( $membership_status['label'] ); ?></option><?php
							endif;
					...
					<input type="button"
					       class="wcm-btn wcm-filter-btn"
					       value="<?php esc_html_e( 'Filter', 'woocommerce-memberships' ); ?>" />
