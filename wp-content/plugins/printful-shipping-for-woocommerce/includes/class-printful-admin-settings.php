...
class Printful_Admin_Settings {
	...
	public static function getIntegrationFields() {
		...
		return array(
			'calculate_tax' => array(
				...
				'type' => 'checkbox',
				'label' => sprintf(
				/* translators: %s is a placeholder for a sales tax link */
	...
	public function render() {
		...
		echo '<form method="post" name="printful_settings" action="' . esc_url( admin_url( 'admin-ajax.php?action=save_printful_settings' ) ) . '&_wpnonce=' . esc_attr(wp_create_nonce( 'printful_settings' )) . '">';

		Printful_Admin::load_template('reconnect', array( 'reconnect_url' => Printful_Admin_Dashboard::instance()->get_connect_url() ));
	 ...
	 * @return array
	 */
	public function setup_fields( $title, $description = '', $fields = array() ) {
		$fieldGroup = array(
			'title'       => $title,
	...
	public function prepare_carriers() {
		...
		$us_carriers[self::CARRIER_REGION_US] = (
			(
				! empty( $_POST[self::CARRIER_REGION_US] ) &&
				! empty( $_POST['_wpnonce'] )
			) &&
			wp_verify_nonce( sanitize_text_field( $_POST['_wpnonce'] ), 'printful_settings' )

		) ? map_deep( wp_unslash( $_POST[self::CARRIER_REGION_US] ), 'sanitize_text_field' ): array();


		$eu_carriers[self::CARRIER_REGION_EU] = (
			(
				! empty( $_POST[self::CARRIER_REGION_EU] ) &&
				! empty( $_POST['_wpnonce'] )
			) &&
			wp_verify_nonce( sanitize_text_field( $_POST['_wpnonce'] ), 'printful_settings' )
		) ? map_deep( wp_unslash( $_POST[self::CARRIER_REGION_EU] ), 'sanitize_text_field' ) : array();
		...
		foreach ( $carrier_regions as $region => $carrier_region ) {
			foreach ( $carrier_region as $carrier_type => $carrier_methods ) {
				foreach ( $carrier_methods as $carrier_method => $carrier_data ) {
					...
					if ( in_array( $carrier_method, isset($saved_carriers[ $region ][ $carrier_type ]) ? $saved_carriers[ $region ][ $carrier_type ] : array() ) ) {
						$is_active = true;
					}
	...
	public static function save_printful_settings() {
		...
		if ( ! empty( $_POST ) ) {
			...
			if ( Printful_Integration::instance()->is_connected(true) ) {
				...
				$request_body = self::instance()->prepare_carriers();
			...
			foreach ($carrier_types as $carrier_type => $carrier_data) {
				foreach ($carrier_data as $key => $carrier) {
					$carrier_item = array(
						...
						'value' => ( true == $carrier[ 'isActive' ] ? 'yes' : 'no' ),
